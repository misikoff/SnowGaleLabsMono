...
export default function SuperScroller({
  options,
  onSelect,
}: {
  ...
  useEffect(() => {
    const filtered = newOptions.filter((option) =>
  ...
  const availableLetters = Array.from(
    new Set(filteredOptions.map((option) => option.letter)),
  )
            ...
            {alphabet.map((letter) => {
              ...
              return (
                <div key={letter} id={letter}>
                  ...
                  {optionsForLetter.map((option, index) => (
                    <div
                      key={index}
